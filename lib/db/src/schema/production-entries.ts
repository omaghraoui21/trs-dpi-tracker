...
export const entryStatusEnum = pgEnum("entry_status", ["draft", "submitted", "validated", "rejected"]);
...
export const productionEntriesTable = pgTable("production_entries", {
  ...
  dailyEntryId: uuid("daily_entry_id").references(() => dailyEntriesTable.id, { onDelete: "set null" }),
...
export const insertProductionEntrySchema = createInsertSchema(productionEntriesTable).omit({ id: true, createdAt: true, updatedAt: true });
