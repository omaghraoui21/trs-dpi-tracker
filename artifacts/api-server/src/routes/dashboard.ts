...
async function batchFetchMetadata(
  ...
  const downtimesByEntry = new Map<
    string,
    Array<{ durationMinutes: number; isPlanned: boolean | null }>
  >();
...
function computeEntryMetrics(
  ...
  const unplannedMinutes = downtimes
    .filter((d) => !d.isPlanned)
    .reduce((s, d) => s + d.durationMinutes, 0);
...
async function getEntriesWithMetrics(from: string, to: string, equipmentId?: string) {
  const filters = [
    ...
    or(
      eq(productionEntriesTable.status, "submitted"),
      eq(productionEntriesTable.status, "validated"),
    )!,
  ];
 ...
 */
async function getDailyBase(
  from: string,
  to: string,
  equipmentId: string,
): Promise<DailyBaseRow[]> {
  const rows = await db
    .select({
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { result: monthlyData, entriesWithMetrics } = await getMonthlyTrsResult(
      from,
      to,
      equipmentId,
      75,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sorted = Array.from(byGroup.entries()).sort(
...
router.get(
  "/dashboard/equipment-comparison",
  requireAuth,
  asyncHandler(async (req, res) => {
    ...
    // 4 queries total regardless of equipment count (vs N×3 before)
    const [allEquipments, allEntries, allDailyRows] = await Promise.all([
      db.select().from(equipmentsTable).where(eq(equipmentsTable.isActive, true)),
      db
        .select()
        .from(productionEntriesTable)
        .where(
      ...
      db
        ...
        .from(dailyEntriesTable)
        .where(and(gte(dailyEntriesTable.entryDate, from), lte(dailyEntriesTable.entryDate, to))),
    ]);

    // Batch downtime + cadence fetches (2 more queries, skipped when no entries)
    const { downtimesByEntry, cadencesByPair } = await batchFetchMetadata(allEntries);

    // Group entries and daily rows by equipment (in memory)
    const entriesByEquip = new Map<string, typeof allEntries>();
    for (const e of allEntries) {
      const arr = entriesByEquip.get(e.equipmentId) ?? [];
      arr.push(e);
      entriesByEquip.set(e.equipmentId, arr);
    }

    const dailyByEquip = new Map<string, DailyBaseRow[]>();
    for (const r of allDailyRows) {
      const tAP = r.pauseMin + r.chsgMin + r.aprMin + r.mqchMin;
      const tR = Math.max(0, r.tOpeningMin - tAP);
      const arr = dailyByEquip.get(r.equipmentId) ?? [];
      arr.push({ tO: r.tOpeningMin, tAP, tR });
      dailyByEquip.set(r.equipmentId, arr);
    }

    const result = allEquipments.map((equip) => {
      const trsObj = parseFloat(equip.trsObjective as unknown as string);
      const equipEntries = entriesByEquip.get(equip.id) ?? [];
      const dailyBase = dailyByEquip.get(equip.id) ?? [];
      const prodMetrics = equipEntries.map((entry) =>
        computeEntryMetrics(entry, downtimesByEntry, cadencesByPair),
      );

      const monthly =
        dailyBase.length > 0
          ? calculateMonthlyTrsV2(dailyBase, prodMetrics, trsObj)
          : calculateMonthlyTrs({ entries: prodMetrics, trsObjective: trsObj });

      const productionDays = new Set(equipEntries.map((e) => e.date)).size;
      return {
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await Promise.all(
      Array.from({ length: 12 }, (_, i) => i + 1).map(async (month) => {
        const { from, to } = getMonthDateRange(month, year);
        const { result: monthly, entriesWithMetrics } = await getMonthlyTrsResult(
          from,
          to,
          equipmentId,
          75,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = Array.from(byWeek.keys())
      .sort((a, b) => a - b)
      .map((weekNum) => {
        const wEntries = byWeek.get(weekNum)!;
        const metrics = wEntries.map((entry) =>
          computeEntryMetrics(entry, downtimesByEntry, cadencesByPair),
        );
        const w = calculateMonthlyTrs({ entries: metrics, trsObjective: 75 });
        return {
          week: weekNum,
          weekLabel: `S${String(weekNum).padStart(2, "0")}`,
