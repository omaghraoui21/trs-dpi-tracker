...
router.get("/equipments", requireAuth, cache30, asyncHandler(async (req, res) => {
  ...
  const rows = includeInactive
    ...
    : await db.select().from(equipmentsTable).where(eq(equipmentsTable.isActive, true)).orderBy(equipmentsTable.name);
...
router.patch("/equipments/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  ...
  const [existing] = await db.select().from(equipmentsTable).where(eq(equipmentsTable.id, params.data.id));
  if (!existing) {
...
router.delete("/equipments/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  ...
  // deactivate
  const [row] = await db.update(equipmentsTable).set({ isActive: false }).where(eq(equipmentsTable.id, id)).returning();
...
router.post("/equipments/:id/reactivate", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  const params = UpdateEquipmentParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  ...
  const [row] = await db.update(equipmentsTable).set({ isActive: true }).where(eq(equipmentsTable.id, params.data.id)).returning();
  writeAudit({
    userId: req.user!.id,
    tableName: "equipments",
    recordId: row.id,
