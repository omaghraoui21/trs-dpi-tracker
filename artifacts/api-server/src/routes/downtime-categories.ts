...
router.get("/downtime-categories", requireAuth, cache30, asyncHandler(async (req, res) => {
  ...
  const rows = includeInactive
    ...
    : await db.select().from(downtimeCategoriesTable).where(eq(downtimeCategoriesTable.isActive, true)).orderBy(downtimeCategoriesTable.code);
...
router.patch("/downtime-categories/:id", requireAuth, requireRole("admin", "supervisor"), asyncHandler(async (req, res) => {
  ...
  const [existing] = await db.select().from(downtimeCategoriesTable).where(eq(downtimeCategoriesTable.id, params.data.id));
  if (!existing) {
...
router.delete("/downtime-categories/:id", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  ...
  const [existing] = await db.select().from(downtimeCategoriesTable).where(eq(downtimeCategoriesTable.id, id));
  ...
  // deactivate
  const [row] = await db.update(downtimeCategoriesTable).set({ isActive: false }).where(eq(downtimeCategoriesTable.id, id)).returning();
...
router.post("/downtime-categories/:id/reactivate", requireAuth, requireRole("admin"), asyncHandler(async (req, res) => {
  const params = UpdateDowntimeCategoryParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  ...
  const [row] = await db.update(downtimeCategoriesTable).set({ isActive: true }).where(eq(downtimeCategoriesTable.id, params.data.id)).returning();
  writeAudit({
    userId: req.user!.id,
    tableName: "downtime_categories",
    recordId: row.id,
