...
router.get("/calendar-events", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (dateFrom) filters.push(gte(annualCalendarEventsTable.dateFrom, dateFrom) as ReturnType<typeof eq>);
  if (dateTo) filters.push(lte(annualCalendarEventsTable.dateTo, dateTo) as ReturnType<typeof eq>);
  ...
  if (eventType) filters.push(eq(annualCalendarEventsTable.eventType, eventType as typeof EVENT_TYPES[number]) as ReturnType<typeof eq>);
  if (equipmentId) {
    filters.push(or(
  ...
  for (const event of relevant) {
    ...
    while (cur <= effectiveEnd) {
      ...
      switch (event.eventType) {
        case "CLOSURE": closureMinutes += minPerDay; break;
        case "HOLIDAY": holidayMinutes += minPerDay; break;
        case "QUALIFICATION": qualificationMinutes += minPerDay; break;
        case "TRIAL": trialMinutes += minPerDay; break;
        case "CLEANING_MAJOR": cleaningMajorMinutes += minPerDay; break;
      }
...
router.post("/calendar-events", requireAuth, requireRole("supervisor", "admin"), asyncHandler(async (req, res) => {
  ...
  if (data.dateTo < data.dateFrom) { res.status(400).json({ error: "dateTo doit être >= dateFrom" }); return; }

  const [row] = await db.insert(annualCalendarEventsTable).values({
...
router.patch("/calendar-events/:id", requireAuth, requireRole("supervisor", "admin"), asyncHandler(async (req, res) => {
  ...
  if (data.dateFrom && data.dateTo && data.dateTo < data.dateFrom) {
    res.status(400).json({ error: "dateTo doit être >= dateFrom" }); return;
  }

  const [updated] = await db.update(annualCalendarEventsTable)
    .set({ ...data })
