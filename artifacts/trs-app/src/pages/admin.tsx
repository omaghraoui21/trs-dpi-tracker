...
import { DeactivateDialog, ShowInactiveToggle } from "@/components/admin";
import {
  useListUsers,
  useCreateUser,
  useUpdateUser,
...
function EquipmentsTab() {
  ...
  const reactivate = useMutation({
    mutationFn: (id: string) => updateEquipment.mutateAsync({ id, data: { isActive: true } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: getListEquipmentsQueryKey() }),
  });

  const allEquipments = equipments ?? [];
  const inactiveCount = allEquipments.filter((e: { isActive: boolean }) => !e.isActive).length;
  const visibleEquipments = showInactive
    ? allEquipments
    : allEquipments.filter((e: { isActive: boolean }) => e.isActive);
  const deactivateItem = allEquipments.find((e: { id: string }) => e.id === deactivateId);

  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", code: "", description: "", equipmentType: "", trsObjective: 75 });
    setSaveError(null);
    setOpen(true);
  };
  const openEdit = (e: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (msg.includes("409") || msg.includes("existe déjà"))
        setSaveError("Un équipement avec ce code existe déjà");
      else setSaveError("Erreur lors de l'enregistrement");
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <SectionHeader
          title="Équipements"
          count={visibleEquipments.length}
          onAdd={openCreate}
      ...
      <TableWrapper>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {["Code", "Nom", "Type", "Objectif TRS", "Statut", ""].map((h) => (
                <th
                  key={h}
                  className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase"
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end items-center gap-1">
                      ...
                      {e.isActive ? (
                        <button
                          className="h-9 w-9 flex items-center justify-center rounded-lg text-red-500 hover:bg-red-500/10"
                          onClick={() => setDeactivateId(e.id)}
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
                      ) : (
                        <button
                          className="h-8 px-2.5 rounded-lg text-xs font-medium text-green-500 border border-green-500/30 hover:bg-green-500/10"
      ...
      {/* Deactivation dialog with dependencies */}
      <DeactivateDialog
        open={!!deactivateId}
        onOpenChange={(v) => {
          if (!v) setDeactivateId(null);
        }}
        entityName="cet équipement"
        entityLabel={deactivateItem?.name ?? ""}
        dependenciesUrl={`/api/equipments/${deactivateId}/dependencies`}
        onConfirm={() => {
          if (deactivateId) deleteEquipment.mutate(deactivateId);
        }}
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            {[
              ...
              { l: "Type d'équipement", k: "equipmentType" },
            ].map(({ l, k }) => (
              <div key={k} className="space-y-1.5">
                <Label>{l}</Label>
                <Input
...
function ProductsTab() {
  ...
  const reactivate = useMutation({
    mutationFn: (id: string) => updateProduct.mutateAsync({ id, data: { isActive: true } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: getListProductsQueryKey() }),
  });

  const allProducts = products ?? [];
  const inactiveCount = allProducts.filter((p: { isActive: boolean }) => !p.isActive).length;
  const visibleProducts = showInactive
    ? allProducts
    : allProducts.filter((p: { isActive: boolean }) => p.isActive);
  const deactivateItem = allProducts.find((p: { id: string }) => p.id === deactivateId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "";
      if (msg.includes("409") || msg.includes("existe déjà"))
        setSaveError("Un produit avec ce code existe déjà");
      else setSaveError("Erreur lors de l'enregistrement");
    }
  };
    ...
    <div className="space-y-4">
      ...
      <TableWrapper>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {["Code", "Nom", "Dosage", "Forme", "Statut", ""].map((h) => (
                <th
                  key={h}
                  className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase"
                >
            ...
            {visibleProducts.map(
              (p: {
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">{p.dosage ?? "—"}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">
      ...
      <DeactivateDialog
        ...
        entityName="ce produit"
        entityLabel={deactivateItem?.name ?? ""}
        dependenciesUrl={`/api/products/${deactivateId}/dependencies`}
        onConfirm={() => {
          if (deactivateId) deleteProduct.mutate(deactivateId);
        }}
...
function CategoriesTab() {
  ...
  const reactivate = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/api/downtime-categories/${id}`, "PATCH", { isActive: true }),
    onSuccess: () => qc.invalidateQueries({ queryKey: getListDowntimeCategoriesQueryKey() }),
  });
  ...
  const visibleCategories = showInactive
    ? allCategories
    : allCategories.filter((c: { isActive?: boolean }) => c.isActive !== false);
  const deactivateItem = allCategories.find((c: { id: string }) => c.id === deactivateId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "";
      if (msg.includes("409") || msg.includes("existe déjà"))
        setSaveError("Une catégorie avec ce code existe déjà");
      else setSaveError("Erreur lors de l'enregistrement");
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <DeactivateDialog
        ...
        entityName="cette catégorie"
        entityLabel={deactivateItem?.label ?? ""}
        dependenciesUrl={`/api/downtime-categories/${deactivateId}/dependencies`}
        onConfirm={() => {
          if (deactivateId) deleteCat.mutate(deactivateId);
        }}
...
function FormulasTab() {
  ...
  const openNewVersion = (f: Formula) => {
    setVersionForm({
  ...
  const handleNewVersion = async () => {
    ...
    setVersionError(null);
    try {
      await apiFetch("/api/formulas", "POST", versionForm);
      qc.invalidateQueries({ queryKey: ["formulas"] });
      setNewVersionOpen(false);
    } catch (e) {
      setVersionError(e instanceof Error ? e.message : "Erreur");
    } finally {
  ...
  const handleValidate = async (id: string) => {
    setValidating(id);
    try {
      await apiFetch(`/api/formulas/${id}/validate`, "POST");
      qc.invalidateQueries({ queryKey: ["formulas"] });
    } catch {
      /* toast handled by global error handler */
    } finally {
      setValidating(null);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-sky-500/10 border border-sky-500/30 rounded-lg px-4 py-3 text-xs text-sky-400 space-y-1">
        <p className="font-semibold">Formules actives NF E 60-182</p>
      ...
      <TableWrapper>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {["Code", "Indicateur", "Formule", "Unité", "v.", "Statut", "Actions"].map((h) => (
                <th
                  key={h}
                  className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase"
                >
                ...
                <td className="px-4 py-3">
                  {f.isActive && (
                    <div className="flex items-center gap-1">
                      <button
                        className="h-8 px-2.5 rounded-lg text-xs gap-1 flex items-center hover:bg-muted border border-border"
                        onClick={() => openTest(f)}
                      >
                      ...
                      {f.validationStatus === "draft" && (
                        <button
                          className="h-8 px-2.5 rounded-lg text-xs gap-1 flex items-center hover:bg-green-500/10 border border-green-500/30 text-green-400"
      ...
      <Dialog open={newVersionOpen} onOpenChange={setNewVersionOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Nouvelle version — {versionForm.indicatorCode}</DialogTitle>
            <DialogDescription className="text-xs">
              Les versions précédentes seront automatiquement dépréciées.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                value={versionForm.formulaExpression}
                onChange={(e) =>
                  setVersionForm((f) => ({ ...f, formulaExpression: e.target.value }))
                }
                className="h-11 font-mono"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                value={versionForm.formulaDescription}
                onChange={(e) =>
                  setVersionForm((f) => ({ ...f, formulaDescription: e.target.value }))
                }
                className="h-11"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                value={versionForm.variablesJson}
                onChange={(e) => setVersionForm((f) => ({ ...f, variablesJson: e.target.value }))}
                className="h-11 font-mono"
                placeholder='["tU","tR"]'
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setVersionForm((f) => ({ ...f, unit: e.target.value }))}
                  className="h-11"
                />
              </div>
              <div className="space-y-1.5">
                <Label>Nom indicateur</Label>
                <Input
                  value={versionForm.indicatorName}
                  onChange={(e) => setVersionForm((f) => ({ ...f, indicatorName: e.target.value }))}
                  className="h-11"
                />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                onChange={(e) => setVersionForm((f) => ({ ...f, changeReason: e.target.value }))}
          ...
          <DialogFooter>
            <Button variant="outline" className="h-11" onClick={() => setNewVersionOpen(false)}>
              Annuler
            </Button>
            <Button
            ...
            >
              {versionSaving ? "Enregistrement..." : "Créer la version"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
