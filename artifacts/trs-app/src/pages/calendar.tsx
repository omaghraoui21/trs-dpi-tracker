...
import {
  Calendar, Plus, Trash2, Pencil, CalendarDays, AlertTriangle,
  FlaskConical, Wrench, Ban, Sun, Loader2, Info, ChevronLeft, ChevronRight,
} from "lucide-react";
...
export type CalendarEventType = "CLOSURE" | "HOLIDAY" | "QUALIFICATION" | "TRIAL" | "CLEANING_MAJOR";
        ...
        <div className="space-y-4 py-1">
          <div>
            ...
            <Select value={form.eventType} onValueChange={v => set("eventType")(v)}>
              ...
              <SelectContent>
                {(Object.entries(EVENT_TYPE_CONFIG) as [CalendarEventType, typeof EVENT_TYPE_CONFIG[CalendarEventType]][]).map(([k, v]) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="date" value={form.dateFrom} onChange={e => set("dateFrom")(e.target.value)} className="h-10" />
            </div>
            <div>
              <Label className="mb-1.5 block text-sm">Date fin</Label>
              <Input type="date" value={form.dateTo} onChange={e => set("dateTo")(e.target.value)} className="h-10" />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Désactivez pour saisir une durée partielle</p>
            </div>
          ...
          <div>
            ...
            <Select value={form.scope} onValueChange={v => set("scope")(v as CalendarEventScope)}>
              ...
              <SelectContent>
                <SelectItem value="SITE" className="py-2.5">Site entier (tous équipements)</SelectItem>
                <SelectItem value="EQUIPMENT" className="py-2.5">Équipement spécifique</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {form.scope === "EQUIPMENT" && (
            <div>
              ...
              <Select value={form.equipmentId} onValueChange={set("equipmentId")}>
                <SelectTrigger className="h-10"><SelectValue placeholder="Sélectionner un équipement" /></SelectTrigger>
                <SelectContent>
                  {equipments.filter(e => (e as unknown as { isActive?: boolean }).isActive !== false).map(e => (
                    <SelectItem key={e.id} value={e.id} className="py-2.5">{e.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Afficher automatiquement les années suivantes</p>
            </div>
            <Switch checked={form.isRecurringAnnual} onCheckedChange={v => set("isRecurringAnnual")(v)} />
          </div>
...
function ImpactCard({ impact }: { impact: CalendarImpact }) {
  const toPercent = (min: number, total: number) => total > 0 ? ((min / total) * 100).toFixed(1) + "%" : "0%";

  return (
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        <Info className="h-4 w-4 text-sky-400" />
        <span className="font-semibold text-sm">Impact OEE — {MONTHS_FR[impact.month - 1]} {impact.year}</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg bg-sky-900/30 border border-sky-700/50 p-3">
          ...
          <p className="text-xs text-sky-400">−{minutesToHours(impact.tODeductionMinutes)} ({toPercent(impact.tODeductionMinutes, impact.totalCalendarMinutes)})</p>
        </div>
        <div className="rounded-lg bg-emerald-900/30 border border-emerald-700/50 p-3">
          ...
          <p className="font-bold text-lg mt-0.5 text-emerald-200">{minutesToHours(impact.totalTR)}</p>
          <p className="text-xs text-emerald-400">−{minutesToHours(impact.tRDeductionMinutes)} ({toPercent(impact.tRDeductionMinutes, impact.totalTO)})</p>
        </div>
      </div>

      {(impact.closureMinutes > 0 || impact.holidayMinutes > 0 || impact.qualificationMinutes > 0 || impact.trialMinutes > 0 || impact.cleaningMajorMinutes > 0) && (
        <div className="space-y-1.5">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Déductions détaillées</p>
          <div className="grid grid-cols-2 gap-2 text-xs">
            {impact.closureMinutes > 0 && (
              <div className="flex items-center justify-between bg-slate-700/30 rounded px-2.5 py-1.5">
                <span className="flex items-center gap-1.5"><Ban className="h-3 w-3 text-slate-400" /> Fermetures (→ tO)</span>
                <span className="font-mono font-medium">{minutesToHours(impact.closureMinutes)}</span>
              </div>
            )}
            {impact.holidayMinutes > 0 && (
              <div className="flex items-center justify-between bg-violet-900/20 rounded px-2.5 py-1.5">
                <span className="flex items-center gap-1.5"><Sun className="h-3 w-3 text-violet-400" /> Jours fériés (→ tO)</span>
                <span className="font-mono font-medium">{minutesToHours(impact.holidayMinutes)}</span>
              </div>
            )}
            {impact.qualificationMinutes > 0 && (
              <div className="flex items-center justify-between bg-amber-900/20 rounded px-2.5 py-1.5">
                <span className="flex items-center gap-1.5"><FlaskConical className="h-3 w-3 text-amber-400" /> Qualifications (→ tR)</span>
                <span className="font-mono font-medium">{minutesToHours(impact.qualificationMinutes)}</span>
              </div>
            )}
            {impact.trialMinutes > 0 && (
              <div className="flex items-center justify-between bg-sky-900/20 rounded px-2.5 py-1.5">
                <span className="flex items-center gap-1.5"><FlaskConical className="h-3 w-3 text-sky-400" /> Essais (→ tR)</span>
            ...
            {impact.cleaningMajorMinutes > 0 && (
              <div className="flex items-center justify-between bg-cyan-900/20 rounded px-2.5 py-1.5">
                <span className="flex items-center gap-1.5"><Wrench className="h-3 w-3 text-cyan-400" /> Nettoyages (→ tR)</span>
                <span className="font-mono font-medium">{minutesToHours(impact.cleaningMajorMinutes)}</span>
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="space-y-0.5">
                    {dayEvents.slice(0, 3).map((ev, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={cn("text-[10px] px-1 py-0.5 rounded truncate font-medium", cfg.bg, cfg.color)}
                          title={ev.label}
                        >
                          {cfg.shortLabel}
                        </div>
...
export default function CalendarPage() {
  ...
  const activeEquipments = equipments.filter(e => (e as unknown as { isActive?: boolean }).isActive !== false);
  ...
  const createMutation = useMutation({
    mutationFn: (form: EventForm) => apiFetch<CalendarEvent>("/api/calendar-events", {
      method: "POST",
      body: JSON.stringify({
        ...
        dateTo: form.dateTo,
        durationMinutesPerDay: form.allDay ? null : (form.durationMinutesPerDay ? parseInt(form.durationMinutesPerDay) : null),
        allDay: form.allDay,
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Select value={String(year)} onValueChange={v => setYear(Number(v))}>
            ...
            <SelectContent>{years.map(y => <SelectItem key={y} value={String(y)} className="py-2.5">{y}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={filterType} onValueChange={v => setFilterType(v as CalendarEventType | "ALL")}>
            <SelectTrigger className="h-10 w-44"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="ALL" className="py-2.5">Tous les types</SelectItem>
              {(Object.entries(EVENT_TYPE_CONFIG) as [CalendarEventType, typeof EVENT_TYPE_CONFIG[CalendarEventType]][]).map(([k, v]) => (
                <SelectItem key={k} value={k} className="py-2.5">{v.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Button onClick={() => { setEditEvent(null); setDialogOpen(true); }} className="h-10 gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        {(Object.entries(EVENT_TYPE_CONFIG) as [CalendarEventType, typeof EVENT_TYPE_CONFIG[CalendarEventType]][]).map(([k, v]) => (
          <div key={k} className={cn("flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium", v.bg, v.color, v.border)}>
            {v.icon} {v.label}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={prevMonth} className="h-9 w-9 flex items-center justify-center rounded-lg border border-border hover:bg-muted transition-colors">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <span className="font-semibold text-base min-w-[140px] text-center">
              {MONTHS_FR[month - 1]} {year}
            </span>
            <button onClick={nextMonth} className="h-9 w-9 flex items-center justify-center rounded-lg border border-border hover:bg-muted transition-colors">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="font-semibold text-sm">
              {MONTHS_FR[month - 1]} {year} — {filteredMonthEvents.length} événement{filteredMonthEvents.length !== 1 ? "s" : ""}
            </p>
          </div>
          ...
          {!isLoading && filteredMonthEvents.length === 0 && (
            <div className="rounded-xl border border-dashed border-border p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Cliquez sur "Nouvel événement" pour planifier</p>
            </div>
          )}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className={cn("text-sm font-semibold leading-tight", cfg.color)}>{ev.label}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {isMultiDay
                          ? `Du ${ev.dateFrom} au ${ev.dateTo}`
                          : ev.dateFrom}
                        {!ev.allDay && ev.durationMinutesPerDay && ` · ${minutesToHours(ev.durationMinutesPerDay)}/j`}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    {deleteConfirm === ev.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        <button className="h-6 px-1.5 rounded bg-red-500 text-white text-xs" onClick={() => deleteMutation.mutate(ev.id)}>Oui</button>
                        <button className="h-6 px-1.5 rounded border border-border text-xs" onClick={() => setDeleteConfirm(null)}>Non</button>
                      </div>
                    ) : (
                      <button
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className={cn("px-2 py-0.5 rounded-full border text-[11px] font-medium", cfg.bg, cfg.color, cfg.border)}>
                    {cfg.impact}
                  </span>
          ...
          {events.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-4 space-y-2 mt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Résumé {year}</p>
              {(["CLOSURE","HOLIDAY","QUALIFICATION","TRIAL","CLEANING_MAJOR"] as CalendarEventType[]).map(t => {
                const count = events.filter(e => e.eventType === t).length;
                if (count === 0) return null;
                const cfg = EVENT_TYPE_CONFIG[t];
                return (
                  <div key={t} className="flex items-center justify-between text-xs">
                    <span className={cn("flex items-center gap-1.5", cfg.color)}>{cfg.icon} {cfg.label}</span>
                    <span className="font-medium">{count} événement{count > 1 ? "s" : ""}</span>
                  </div>
                );
              })}
            </div>
      ...
      <EventFormDialog
        open={dialogOpen}
        onOpenChange={open => { setDialogOpen(open); if (!open) setEditEvent(null); }}
        initial={editEvent ? {
          ...
          durationMinutesPerDay: editEvent.durationMinutesPerDay ? String(editEvent.durationMinutesPerDay) : "",
