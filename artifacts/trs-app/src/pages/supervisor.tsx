    ...
    <div className="space-y-1.5">
      ...
      <div className="h-2.5 bg-muted rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${pct}%`, backgroundColor: trsColor(value) }}
        />
      </div>
    ...
    <div className="space-y-1">
      <div className="flex justify-between items-center text-xs">
        ...
        <span className="font-medium tabular-nums">
          {pct.toFixed(1)}% · {Math.round(minutes)} min
        </span>
      </div>
...
function LotFicheModal({
  ...
  async function saveQty() {
    ...
    if (isNaN(produced) || isNaN(conforming)) {
      setQtyError("Valeurs invalides");
      return;
    }
    if (conforming > produced) {
      setQtyError("Conforme > Produit total");
      return;
    }
    setQtyError("");
    setSavingQty(true);
    try {
      await customFetch(`/api/production-entries/${entry.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          quantityProduced: produced,
          quantityConforming: conforming,
          quantityRejected: produced - conforming,
        }),
      });
      setEntry((e) => ({
        ...e,
  ...
  async function saveDtComment() {
    ...
    setSavingDt(true);
    try {
      await customFetch(`/api/downtime-events/${editingDtId}`, {
        method: "PATCH",
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0">
        ...
        <div className="sticky top-0 z-10 bg-card border-b border-border px-6 py-4 flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn("text-[11px] font-semibold px-2 py-0.5 rounded-full", statusInfo.cls)}
              >
                {statusInfo.label}
              </span>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              {entry.date} · {entry.equipmentName ?? "?"}
            </p>
          </div>
          <button
            onClick={onClose}
            className="h-7 w-7 rounded-full bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors shrink-0 mt-0.5"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              { icon: <Zap className="h-3.5 w-3.5" />, label: "Poste", value: entry.shift ?? "—" },
              {
                ...
                label: "Arrêts total",
                value: totalDtMin > 0 ? fmtDur(totalDtMin) : "—",
              },
              ...
              {[
                {
                  label: "Produit (total)",
                  key: "produced",
                  value: entry.quantityProduced ?? 0,
                  edit: qtyProduced,
                  setter: setQtyProduced,
                },
                {
                  label: "Conforme",
                  key: "conforming",
                  value: entry.quantityConforming ?? 0,
                  edit: qtyConforming,
                  setter: setQtyConforming,
                },
                {
                  label: "Rebuté",
                  key: "rejected",
                  value: editingQty
                    ? (parseInt(qtyProduced) || 0) - (parseInt(qtyConforming) || 0)
                ...
                <div key={key} className="bg-muted/30 rounded-lg p-3">
                  ...
                  {editingQty && setter ? (
                    ...
                    <div
                      className={cn(
                        "text-lg font-bold",
                        key === "rejected" && value > 0 ? "text-red-400" : "",
                      )}
                    >
            ...
            <div>
                            ...
                            >
                              <span
                                className={cn(
                                  ...
                                  d.comment ? "text-foreground" : "text-muted-foreground/40 italic",
                                )}
                              >
                                {d.comment ?? "Ajouter…"}
                              </span>
              ...
              <div className="flex items-center gap-4 mt-2 text-[11px] text-muted-foreground">
                <span className="flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded-sm bg-blue-500/60 inline-block" /> Planifié
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded-sm bg-red-500/60 inline-block" /> Non
                  planifié
                </span>
              </div>
            </div>
        ...
        <Dialog open={anomalyOpen} onOpenChange={setAnomalyOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Anomalie observée</DialogTitle>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  onAction(entry.id, "reject", anomalyComment);
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground truncate">
              {entry.productName} · Lot {entry.batchNumber} · {entry.shift}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0 ml-2">
          {entry.trsMetrics && (
            <span
              ...
              style={{ color: trsColor(entry.trsMetrics.TRS * 100) }}
            >
        ...
        <div className="border-t border-border px-4 py-3 bg-muted/30 space-y-2">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {[
              ...
              <div
                key={label}
                className="text-center bg-card border border-border rounded-lg py-2 px-1"
              >
                <div className="text-xs text-muted-foreground">{label}</div>
                <div className="text-base font-bold" style={{ color: trsColor(v * 100) }}>
                  {(v * 100).toFixed(1)}%
                </div>
...
export default function SupervisorPage() {
  ...
  const { data: paretoDetail } = useGetDowntimePareto({
    month,
    year,
    equipmentId,
    groupBy: "detail",
  });
  const { data: paretoFamille } = useGetDowntimePareto({
    month,
    year,
    equipmentId,
    groupBy: "famille",
  ...
  const paretoData =
    paretoTab === "detail" ? paretoDetail : paretoTab === "famille" ? paretoFamille : paretoType;
      ...
      { id, data: { action, comment } },
      {
        onSuccess: () => {
          ...
          qc.invalidateQueries({
            queryKey: getGetDashboardSummaryQueryKey({ month, year, equipmentId }),
          });
        },
      },
    );
  ...
  const losses = useMemo(() => {
    ...
    const { totalTR, totalTF, totalTN, totalTU, totalDowntimePlanned, totalDowntimeUnplanned } =
      monthlyKpis;
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            className="h-11 gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <KpiCard
          label="TRS Consolidé"
          value={summary?.currentMonthTrs ?? null}
          target={summary?.trsObjective}
        />
        <KpiCard
          label="TRG (tU/tO)"
          value={monthlyKpis?.TRG ?? null}
          color={trsColor(monthlyKpis?.TRG ?? null)}
        />
        <KpiCard
          label="Disponibilité (DO)"
          value={summary?.currentMonthDO ?? null}
          color={trsColor(summary?.currentMonthDO ?? null)}
        />
        <KpiCard
          label="Performance (TP)"
          value={summary?.currentMonthTP ?? null}
          color={trsColor(summary?.currentMonthTP ?? null)}
        />
        <KpiCard
          label="Qualité (TQ)"
          value={summary?.currentMonthTQ ?? null}
          color={trsColor(summary?.currentMonthTQ ?? null)}
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {
            label: "À réviser",
            value: summary?.pendingValidations ?? 0,
            color: "text-sky-400",
            icon: <Clock className="h-5 w-5" />,
          },
          {
            label: "Jours de prod.",
            value: summary?.productionDays ?? 0,
            color: "text-emerald-400",
            icon: <TrendingUp className="h-5 w-5" />,
          },
          {
            label: "Revues",
            value: summary?.validatedEntries ?? 0,
          ...
          <div className="bg-card border border-border rounded-xl p-5 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold" style={{ color: trsColor(monthlyKpis?.trs ?? null) }}>
                  {trsLabel(monthlyKpis?.trs ?? null)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">TRG (tU/tO)</span>
                <span className="font-bold" style={{ color: trsColor(monthlyKpis?.TRG ?? null) }}>
                  {trsLabel(monthlyKpis?.TRG ?? null)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">TRS = DO × TP × TQ</span>
                <span className="font-bold" style={{ color: trsColor(doTPTQ) }}>
                  {trsLabel(doTPTQ)}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold" style={{ color: trsColor(monthlyKpis?.TP ?? null) }}>
                  {trsLabel(monthlyKpis?.TP ?? null)}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-3">
              ...
              <div className="space-y-2.5">
                <LossBar
                  ...
                  minutes={losses.planned}
                  totalRef={losses.totalRef}
                  color="#3b82f6"
                />
                <LossBar
                  label="Arrêts non planifiés"
                  minutes={losses.unplanned}
                  totalRef={losses.totalRef}
                  color="#ef4444"
                />
                <LossBar
                ...
                <LossBar
                  label="Non-qualité"
                  minutes={losses.quality}
                  totalRef={losses.totalRef}
                  color="#eab308"
                />
                <LossBar
                  label="Temps utile"
                  minutes={losses.useful}
                  totalRef={losses.totalRef}
                  color="#22c55e"
                />
              </div>
        ...
        {dailyTrs && dailyTrs.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={dailyTrs} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              />
              <ReferenceLine
                y={dailyTrs[0]?.trsObjective ?? 75}
                stroke="#94a3b8"
                strokeDasharray="4 4"
                label={{ value: "Obj.", position: "right", fontSize: 10, fill: "#94a3b8" }}
              />
              <Line
                dataKey="trs"
                name="TRS"
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          {paretoData && paretoData.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart
                ...
                layout="vertical"
                margin={{ top: 0, right: 40, left: 65, bottom: 0 }}
              >
                <CartesianGrid
                  strokeDasharray="3 3"
                ...
                <Tooltip
                  formatter={(v: number, _n, p) => [
                    `${v} min (${p.payload.occurrences}×)`,
                    p.payload.categoryLabel,
                  ]}
                  contentStyle={{
                    background: "hsl(var(--card))",
                    border: "1px solid hsl(var(--border))",
                ...
                <Bar dataKey="totalMinutes" radius={[0, 4, 4, 0]}>
                  {paretoData.slice(0, 10).map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.isPlanned ? "#3b82f6" : "#ef4444"}
                      fillOpacity={0.85}
                    />
                  ))}
                </Bar>
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-blue-500 inline-block" /> Planifié
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-red-500 inline-block" /> Non planifié
            </span>
          </div>
        </div>
