...
import {
  Play, StopCircle, Clock, Plus, X, CheckCircle2, AlertTriangle,
  ChevronLeft, Gauge, PackageOpen, Timer, Edit3, AlarmClock, ChevronDown, Droplets,
} from "lucide-react";
...
interface PosteDef { label: string; shift: string; start: string; end: string; }
...
const EQUIP_CFG: Record<string, EquipCfg> = {
  A27: { unit: "gélules",  lotSize: 360_000, increments: [10_000, 50_000, 100_000, 360_000], cadenceInMin: true  },
  A28: { unit: "blisters", lotSize:  36_000, increments: [ 1_000,  5_000,  10_000,  36_000], cadenceInMin: false },
};
const DEFAULT_CFG: EquipCfg = { unit: "unités", lotSize: 0, increments: [100, 500, 1_000, 5_000], cadenceInMin: false };
...
function computeWeightedCadence(
  base: number, changes: { time: string; value: number }[],
...
function computeTrs(params: {
  shiftStart: string; shiftEnd: string;
  produced: number; conforming: number;
  downtimes: DowntimeEvent[]; cadence: number;
}) {
  ...
  const planned = closed.filter(d => d.categoryIsPlanned).reduce((s, d) => s + d.durationMinutes, 0);
  const unplanned = closed.filter(d => !d.categoryIsPlanned).reduce((s, d) => s + d.durationMinutes, 0);
...
function ArrêtModal({
  open, mode, categories, onClose, onConfirmLive, onConfirmMicro,
}: {
  open: boolean;
  ...
  useEffect(() => { if (open) { setCategoryId(""); setCustomMin(""); setError(""); } }, [open]);

  const effectiveMin = mode === "live" ? null : (typeof mode === "number" ? mode : parseInt(customMin, 10) || null);
  const isMicro = mode !== "live";

  async function handleConfirm() {
    if (!categoryId) { setError("Veuillez sélectionner une catégorie"); return; }
    if (isMicro && (!effectiveMin || effectiveMin <= 0)) { setError("Durée invalide"); return; }
    setLoading(true);
    setError("");
    try {
      if (!isMicro) {
  ...
  return (
    <Dialog open={open} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {isMicro ? `Micro-arrêt${typeof mode === "number" ? ` (${mode} min)` : ""}` : "Démarrer un arrêt"}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
                ...
                {categories.map(c => (
                  <SelectItem key={c.id} value={c.id} className="py-3">
                    <span className="flex items-center gap-2">
                      <span className={cn("w-2 h-2 rounded-full shrink-0", c.isPlanned ? "bg-blue-500" : "bg-red-500")} />
                      [{c.code}] {c.label}
                    </span>
                  </SelectItem>
                ))}
          ...
          {isMicro && typeof mode !== "number" && (
            <div className="space-y-2">
              ...
              <Input value={customMin} onChange={e => setCustomMin(e.target.value)} type="number" min={1} max={60} placeholder="Ex: 8" className="h-11" />
            </div>
          )}
...
// ─── Lot Active Tracker ───────────────────────────────────────────────────────
function LotActiveTracker({ lotId, onClosed, onBack }: { lotId: string; onClosed: () => void; onBack: () => void }) {
  const today = new Date().toISOString().split("T")[0];
  const qc = useQueryClient();

  const { data: entries } = useListProductionEntries({ dateFrom: today, dateTo: today });
  const entry = useMemo(() => (entries ?? []).find(e => e.id === lotId) as ProductionEntryWithDetails | undefined, [entries, lotId]);
  ...
  const [quickCatModal, setQuickCatModal] = useState<{ categoryId: string; label: string } | null>(null);
  ...
  const loadDowntimes = useCallback(async () => {
    try {
      const data = await apiFetch<DowntimeEvent[]>(`/downtime-events?entryId=${lotId}`);
      setDowntimes(data);
    } catch { /* silent */ }
  }, [lotId]);
  ...
  const openDowntime = useMemo(() => downtimes.find(d => d.status === "open") ?? null, [downtimes]);

  useEffect(() => {
    if (!openDowntime) { setOpenDtSec(0); return; }
    function calc() {
  ...
  const baseCadence = useMemo(() => {
    const c = (allCadences ?? []).find(c => c.productId === entry?.productId && c.equipmentId === entry?.equipmentId);
    return c ? Number(c.validatedCadence) : 0;
  }, [allCadences, entry]);

  const currentCadence = useMemo(() => {
    return cadenceChanges.length > 0 ? cadenceChanges[cadenceChanges.length - 1].value : baseCadence;
  ...
  const categories = useMemo(
    () => (allCategories ?? []).map(c => ({
      ...
      shortcutEquipments: (c as unknown as { shortcutEquipments?: string | null }).shortcutEquipments ?? null,
    })),
    [allCategories],
  );
  ...
  const quickDtCodes = useMemo(
    () => categories
      .filter(c => {
        ...
        return c.shortcutEquipments.split(",").map(s => s.trim()).includes(equipCode);
      })
      .map(c => c.code),
    [categories, equipCode],
  );

  function addProduced(n: number) { setProduced(p => p + n); setDirty(true); }

  async function saveQty() {
    if (!entry) return;
    setSaving(true);
    try {
      await updateEntry.mutateAsync({ id: lotId, data: { quantityProduced: produced, quantityConforming: conforming, quantityRejected: Math.max(0, produced - conforming) } });
      setDirty(false);
      qc.invalidateQueries({ queryKey: getListProductionEntriesQueryKey({ dateFrom: today, dateTo: today }) });
    } finally { setSaving(false); }
  }

  async function startArrêt(categoryId: string) {
    const data = await apiFetch<DowntimeEvent>("/downtime-events/start", {
      method: "POST",
  ...
  async function stopArrêt() {
    ...
    const data = await apiFetch<DowntimeEvent>(`/downtime-events/${openDowntime.id}/stop`, {
      method: "PATCH",
  ...
  async function addMicroArrêt(categoryId: string, _minutes: number, startTime: string, comment?: string) {
    const endTime = nowHHMM();
    const data = await apiFetch<DowntimeEvent>("/downtime-events", {
      method: "POST",
      body: JSON.stringify({ entryId: lotId, categoryId, startTime, endTime, ...(comment ? { comment } : {}) }),
    });
    setDowntimes(prev => [...prev, data]);
  }

  async function deleteArrêt(id: string) {
    await apiFetch(`/downtime-events/${id}`, { method: "DELETE" });
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-2xl mx-auto pb-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-lg font-bold truncate">{entry.equipmentName ?? "—"} · Lot {entry.batchNumber}</h1>
          <p className="text-sm text-muted-foreground truncate">{entry.productName ?? "—"} · {entry.shift}</p>
        </div>
      ...
      {isPauseTime && !pauseDismissed && !openDowntime && (
        <div className="bg-amber-500/10 border border-amber-500/40 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-400">Rappel pause déjeuner (12h–13h)</p>
              <p className="text-xs text-muted-foreground">Pensez à déclarer un arrêt planifié si la machine est à l'arrêt.</p>
            </div>
          </div>
          <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-foreground" onClick={() => setPauseDismissed(true)}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      )}
        ...
        <div className="bg-red-500/10 border border-red-500/40 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">{openDowntime.categoryLabel ?? "—"}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-xl font-bold tabular-nums text-red-400">{fmtSeconds(openDtSec)}</span>
            <Button size="sm" className="bg-red-600 hover:bg-red-500 text-white h-10 px-4 font-semibold" onClick={stopArrêt}>
      ...
      <div className="bg-card border border-border rounded-xl p-4 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Quantités · {equipCfg.unit}</h2>
          <div className="flex items-center gap-1.5">
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => {
              setCadenceInput(equipCfg.cadenceInMin ? String(Math.round(currentCadence / 60) || "") : String(currentCadence || ""));
        ...
        {cadenceChanges.length > 0 && (
          <div className="flex gap-1.5 flex-wrap">
            {cadenceChanges.map((ch, i) => (
              <span key={i} className="text-xs bg-sky-500/10 text-sky-400 border border-sky-500/20 rounded-full px-2 py-0.5 font-mono">
        ...
        <div className="grid grid-cols-4 gap-2">
          {equipCfg.increments.map(n => (
            <Button key={n} variant="outline" className="h-12 font-semibold text-sm" onClick={() => addProduced(n)} disabled={!!openDowntime}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Input
              type="number" min={0}
              value={produced}
              onChange={e => { setProduced(Math.max(0, parseInt(e.target.value, 10) || 0)); setDirty(true); }}
          ...
          <div className="space-y-1">
            ...
            <Input
              type="number" min={0} max={produced}
              value={conforming}
              onChange={e => { setConforming(Math.max(0, Math.min(produced, parseInt(e.target.value, 10) || 0))); setDirty(true); }}
              className="h-11 text-center text-base font-semibold"
            />
          </div>
        </div>
        ...
        {equipCfg.lotSize > 0 && (
          <div className="space-y-1">
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <span className="tabular-nums">{produced.toLocaleString("fr-FR")} / {equipCfg.lotSize.toLocaleString("fr-FR")} · {Math.round(Math.min(100, produced / equipCfg.lotSize * 100))}%</span>
            </div>
            <div className="h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-sky-500 rounded-full transition-all duration-300" style={{ width: `${Math.min(100, produced / equipCfg.lotSize * 100)}%` }} />
            </div>
          </div>
        )}

        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">Rebus: <span className="font-medium text-foreground">{Math.max(0, produced - conforming)} {equipCfg.unit}</span></span>
          <Button
            className={cn("h-10 px-5", dirty ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}
      ...
      <div className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        {quickDtCodes.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <span className="text-xs text-muted-foreground self-center whitespace-nowrap">Raccourcis :</span>
            {quickDtCodes
              ...
              .map(cat => (
                <Button
                  ...
                  disabled={!!openDowntime}
                  onClick={() => { setQuickCatMin("5"); setQuickCatModal({ categoryId: cat!.id, label: `[${cat!.code}] ${cat!.label}` }); }}
                >
                  {cat!.code}
                </Button>
              ))}
        ...
        {closedDt.length > 0 ? (
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {closedDt.map(d => (
              <div key={d.id} className="flex items-center justify-between text-sm border border-border rounded-lg px-3 py-2 bg-muted/30">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={cn("w-2 h-2 rounded-full shrink-0", d.categoryIsPlanned ? "bg-blue-400" : "bg-red-400")} />
                  <span className="truncate">{d.categoryLabel ?? d.categoryCode ?? "—"}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-muted-foreground tabular-nums">{d.startTime}→{d.endTime}</span>
                  <Badge variant="outline" className="text-xs">{fmtDur(d.durationMinutes)}</Badge>
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-500" onClick={() => deleteArrêt(d.id)}>
      ...
      <Dialog open={showCadenceModal} onOpenChange={setShowCadenceModal}>
        <DialogContent className="max-w-xs">
          <DialogHeader><DialogTitle>Changement de cadence</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              Enregistre la nouvelle cadence à partir de maintenant. L'impact TRS est calculé au prorata du temps.
            </p>
            <div className="space-y-1">
              ...
              <Input
                ...
                type="number" min={1} className="h-11"
            ...
            {cadenceChanges.length > 0 && (
              <div className="border border-border rounded-lg p-2 bg-muted/30 space-y-1">
                ...
                {[{ time: entry?.shiftStart ?? "08:00", value: baseCadence }, ...cadenceChanges].map((ch, i) => (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setCadenceChanges([]); setShowCadenceModal(false); }}>Réinitialiser</Button>
            <Button onClick={() => {
              const n = parseInt(cadenceInput, 10);
              if (n > 0) setCadenceChanges(prev => [...prev, { time: nowHHMM(), value: equipCfg.cadenceInMin ? n * 60 : n }]);
      ...
      <Dialog open={quickCatModal !== null} onOpenChange={v => { if (!v) { setQuickCatModal(null); setQuickCatComment(""); } }}>
        <DialogContent className="max-w-xs">
          <DialogHeader><DialogTitle className="text-sm leading-snug">{quickCatModal?.label}</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setQuickCatModal(null); setQuickCatComment(""); }}>Annuler</Button>
            <Button onClick={async () => {
              ...
              await addMicroArrêt(quickCatModal.categoryId, min, startTime, quickCatComment.trim() || undefined);
      ...
      <Dialog open={showCloture} onOpenChange={setShowCloture}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Clôturer le lot ?</DialogTitle></DialogHeader>
          <div className="py-2 space-y-2">
            <p className="text-sm text-muted-foreground">
              Le lot sera clôturé et intégré au calcul TRS. Cette action est irréversible.
            </p>
            <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-1">
              <div className="flex justify-between"><span className="text-muted-foreground">Produit total</span><span className="font-medium">{produced} u.</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Conforme</span><span className="font-medium">{conforming} u.</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Arrêts</span><span className="font-medium">{closedDt.length} ({fmtDur(totalArrêtMin)})</span></div>
              {trs && <div className="flex justify-between"><span className="text-muted-foreground">TRS estimé</span><span className="font-bold" style={{ color: trsHex(trs.TRS) }}>{(trs.TRS * 100).toFixed(1)}%</span></div>}
            </div>
            {dirty && <p className="text-xs text-amber-500 flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> Quantités non enregistrées — elles seront sauvegardées à la clôture.</p>}
            {clotureError && <p className="text-sm text-red-500">{clotureError}</p>}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowCloture(false)}>Annuler</Button>
            <Button
...
// ─── Lot Start Form ───────────────────────────────────────────────────────────
function LotStartForm({ onStarted, onCancel }: { onStarted: (id: string) => void; onCancel: () => void }) {
  const today = new Date().toISOString().split("T")[0];
  ...
  useEffect(() => {
    ...
    apiFetch<{ suggestion: string }>(`/production-entries/next-batch-number?productId=${encodeURIComponent(productId)}`)
      .then(d => { setBatchNumber(d.suggestion); setBatchSuggested(true); })
      .catch(() => {});
  ...
  useEffect(() => {
    if (activityType === "nettoyage" && nettProduct) {
      ...
      apiFetch<{ suggestion: string }>(`/production-entries/next-batch-number?productId=${encodeURIComponent(nettProduct.id)}`)
        .then(d => { setBatchNumber("NT" + d.suggestion); setBatchSuggested(true); })
        .catch(() => {});
  ...
  async function handleStart() {
    if (!equipmentId) { setError("Sélectionner un équipement"); return; }
    if (activityType === "production") {
      if (!productId) { setError("Sélectionner un produit"); return; }
      if (!batchNumber.trim()) { setError("Saisir un numéro de lot"); return; }
    } else {
      if (!nettProduct) { setError("Produit Nettoyage introuvable — contactez l'administrateur"); return; }
    }
    setError("");
    try {
      const result = await createEntry.mutateAsync({
        data: {
          ...
          batchNumber: batchNumber.trim() || (activityType === "nettoyage" ? `NT${today.replace(/-/g, "")}` : ""),
          shift: poste.shift,
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-lg mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold">{activityType === "nettoyage" ? "Démarrer un nettoyage" : "Démarrer un lot"}</h1>
      ...
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={cn(
                ...
                activityType === "production" ? "border-primary bg-primary/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/50",
              )}
            >
              <PackageOpen className="h-4 w-4 shrink-0" /> Production
              {activityType === "production" && <CheckCircle2 className="h-3.5 w-3.5 text-primary ml-auto" />}
            </button>
            <button
              onClick={() => setActivityType("nettoyage")}
              className={cn(
                "p-3 rounded-xl border text-sm font-medium transition-colors flex items-center gap-2 justify-center",
                activityType === "nettoyage" ? "border-cyan-500 bg-cyan-500/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-cyan-500/50",
              )}
            >
              <Droplets className="h-4 w-4 shrink-0" /> Nettoyage
              {activityType === "nettoyage" && <CheckCircle2 className="h-3.5 w-3.5 text-cyan-500 ml-auto" />}
            </button>
          </div>
        </div>
          ...
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-3 flex items-start gap-3 text-sm">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                Lot auto : <span className="font-mono text-foreground">{batchNumber || "génération…"}</span>
                {batchSuggested && <span className="text-cyan-400 ml-1">· suggéré automatiquement</span>}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <Select value={equipmentId} onValueChange={setEquipmentId}>
            ...
            <SelectContent>
              {(equipments ?? []).filter(e => e.isActive !== false).map(e => (
                <SelectItem key={e.id} value={e.id} className="py-3">[{e.code}] {e.name}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
          ...
          <>
            <div className="space-y-2">
              ...
              <Select value={productId} onValueChange={setProductId}>
                <SelectTrigger className="h-12"><SelectValue placeholder="Sélectionner…" /></SelectTrigger>
                <SelectContent>
                  {(products ?? []).filter(p => p.isActive !== false && (p as { code?: string }).code !== "NETT").map(p => (
                    <SelectItem key={p.id} value={p.id} className="py-3">[{(p as { code?: string }).code}] {p.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2">
                Numéro de lot
                {batchSuggested && <span className="text-xs text-sky-400 font-normal">· suggéré automatiquement</span>}
              </Label>
              <Input
                value={batchNumber}
                onChange={e => { setBatchNumber(e.target.value); setBatchSuggested(false); }}
                className="h-12 text-base font-mono tracking-widest"
                placeholder={`Ex: ${new Date().getFullYear().toString().slice(-2)}001`}
              />
              <p className="text-xs text-muted-foreground">Format AAXXX — {new Date().getFullYear().toString().slice(-2)}001, {new Date().getFullYear().toString().slice(-2)}002, …</p>
            </div>
          </>
        )}
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={() => { setShiftMode("standard"); setPosteIdx(0); setShowOtherShifts(false); }}
            className={cn(
              "w-full text-left p-3 rounded-xl border text-sm transition-colors flex items-center justify-between",
              shiftMode === "standard" ? "border-primary bg-primary/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/50",
            )}
          >
            <div>
          ...
          >
            <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", showOtherShifts && "rotate-180")} />
            Horaires exceptionnels / Ramadan
            {shiftMode !== "standard" && <span className="ml-1 text-xs text-primary font-medium">({SHIFT_MODES[shiftMode].label})</span>}
          </button>
            ...
            <div className="grid grid-cols-1 gap-2 rounded-xl border border-border/60 bg-muted/20 p-3">
              {(Object.entries(SHIFT_MODES) as [ShiftMode, typeof SHIFT_MODES[ShiftMode]][])
                ...
                .map(([k, v]) => (
                  <button
                    key={k}
                    onClick={() => { setShiftMode(k); setPosteIdx(0); }}
                    className={cn(
                      "text-left p-3 rounded-lg border text-sm transition-colors",
                      shiftMode === k ? "border-primary bg-primary/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/50",
                    )}
                  >
              ...
              {mode.postes.map((p, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    posteIdx === i ? "border-primary bg-primary/10" : "border-border bg-card text-muted-foreground",
                  )}
                >
                  {p.label}
                  <div className="text-xs font-normal mt-0.5">{p.start}–{p.end}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="bg-muted/50 rounded-xl p-3 flex items-center gap-3 text-sm">
          ...
          <span className="text-muted-foreground">Poste: <span className="font-medium text-foreground">{poste.shift}</span></span>
          <span className="text-muted-foreground ml-auto">{poste.start}–{poste.end} ({fmtDur(durationMin(poste.start, poste.end))})</span>
        </div>
        ...
        {activityType === "production" && startCfg.lotSize > 0 && (
          <div className="bg-sky-500/5 border border-sky-500/20 rounded-xl p-3 flex items-center gap-3 text-sm">
            ...
            <span className="text-muted-foreground">{startCfg.lotSize.toLocaleString("fr-FR")} {startCfg.unit}</span>
          </div>
        )}
      ...
      <Button
        className={cn("w-full h-14 font-bold text-base", activityType === "nettoyage" && "bg-cyan-600 hover:bg-cyan-700 text-white")}
        onClick={handleStart}
    ...
    <div className="p-4 md:p-6 space-y-5 max-w-2xl mx-auto">
      ...
      {active.length > 0 && (
        <div className="space-y-2">
          ...
          {active.map(e => (
            <div key={e.id} className="bg-amber-500/10 border border-amber-500/40 rounded-xl p-4 flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="font-medium">{(e as ProductionEntryWithDetails).equipmentName ?? "—"} · {(e as ProductionEntryWithDetails).productName ?? "—"}</div>
                <div className="text-sm text-muted-foreground">Lot {e.batchNumber} · {e.shift}</div>
                <div className="text-xs text-muted-foreground mt-0.5">{e.quantityProduced} u. · {e.quantityConforming} conf.</div>
              </div>
              <Button className="h-11 bg-amber-500 hover:bg-amber-400 text-white font-semibold shrink-0 gap-2" onClick={() => onResume(e.id)}>
      ...
      {closed.length > 0 && (
        <div className="space-y-2">
          ...
          {closed.map(e => {
            const statusLabel = e.status === "submitted" ? "Clôturé" : e.status === "validated" ? "Revu ✓" : "Corrigé";
            const statusClass = e.status === "submitted" ? "bg-sky-500/20 text-sky-400 border-sky-500/30" : e.status === "validated" ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-orange-500/20 text-orange-400 border-orange-500/30";
            return (
              <div key={e.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-medium">{(e as ProductionEntryWithDetails).equipmentName ?? "—"} · {(e as ProductionEntryWithDetails).productName ?? "—"}</div>
                  <div className="text-sm text-muted-foreground">Lot {e.batchNumber} · {e.shift}</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{e.quantityProduced} u. · {e.quantityConforming} conf.</div>
                </div>
                <span className={cn("text-xs font-medium px-2.5 py-1 rounded-full border shrink-0", statusClass)}>{statusLabel}</span>
              </div>
            );
          })}
        </div>
      ...
      {!isLoading && (entries ?? []).length === 0 && (
        <div className="border border-dashed border-border rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Cliquez sur "Nouveau lot" pour commencer</p>
        </div>
      )}
    </div>
  );
...
export default function EntryPage() {
  ...
  if (view === "start") {
    return (
      <LotStartForm
        onStarted={(id) => { setActiveLotId(id); setView("active"); }}
  ...
  if (view === "active" && activeLotId) {
    return (
      <LotActiveTracker
        lotId={activeLotId}
        onClosed={() => { setActiveLotId(null); setView("list"); }}
        onBack={() => setView("list")}
      />
    );
  }
  ...
  return (
    <LotListView
      onNew={() => setView("start")}
      onResume={(id) => { setActiveLotId(id); setView("active"); }}
    />
  );
}
