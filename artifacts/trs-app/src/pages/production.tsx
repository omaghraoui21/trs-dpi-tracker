...
interface ProductionEntry {
  ...
  status: string;
  trsMetrics?: { TRS?: number; TRG?: number; DO?: number; TP?: number; TQ?: number; tO?: number };
}

// --- Équipements DPI avec couleurs ---
const DPI_EQUIPMENTS = [
  {
    key: "Géluleuse HH (A27)",
    label: "Géluleuse HH",
    sublabel: "Local A27",
    icon: "⚙️",
    color: "blue",
...
function severityBadge(s: "info" | "warning" | "critical") {
  ...
  return (
    <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", map[s])}>{label[s]}</span>
  );
}
...
function EquipmentCard({
  ...
  return (
    <div className={cn("rounded-xl border p-4 flex flex-col gap-3", colors.bg, colors.border)}>
      ...
      {!hasActivity && (
        <div className="text-xs text-muted-foreground italic">Aucune activité planifiée</div>
      )}
      ...
      {hasActivity && !hasSpecial && (
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            ...
            >
              {realized.toLocaleString("fr-FR")} /{" "}
              {planned > 0 ? planned.toLocaleString("fr-FR") : "—"}{" "}
              {productionPlan?.plannedUnit ?? ""}
            </span>
          </div>
          ...
          {activeEntry?.trsMetrics?.TRG != null && (
            <div className="text-xs flex items-center justify-between">
              <span className="text-muted-foreground">
                TRG <span className="text-muted-foreground/50">(tU/tO)</span>
              </span>
              <span className={cn("font-bold", trsColor(activeEntry.trsMetrics.TRG * 100))}>
                {(activeEntry.trsMetrics.TRG * 100).toFixed(1)}%
              </span>
            </div>
          )}
        </div>
...
function NotificationRow({
  ...
  n: NotificationItem;
  onAction: (id: number, action: "acknowledge" | "close", comment?: string) => void;
}) {
          ...
          {n.status === "open" && (
            <>
              ...
              <Button
                ...
                onClick={() => onAction(n.id, "close", comment || undefined)}
              >
      ...
      {expanded && n.status === "open" && (
        <div className="pl-6 flex gap-2">
          ...
          <Button
            ...
            className="h-7 text-xs"
            onClick={() => {
              onAction(n.id, "close", comment || undefined);
              setExpanded(false);
            }}
          >
            Clôturer
...
export default function ProductionPage() {
  ...
  const {
    ...
    queryFn: () => customFetch(`/api/planning?date=${selectedDate}`),
  ...
  const { data: entries, isLoading: entriesLoading } = useQuery<ProductionEntry[]>({
    ...
    queryFn: () =>
      customFetch(
        `/api/production-entries?dateFrom=${selectedDate}&dateTo=${selectedDate}&status=validated,submitted`,
      ),
    refetchInterval: 30000,
  });
  ...
  const { data: notifications, refetch: refetchNotifs } = useQuery<NotificationItem[]>({
    ...
    queryFn: () =>
      customFetch(`/api/notifications${statusFilter ? `?status=${statusFilter}` : ""}`),
    refetchInterval: 30000,
  });
  ...
  const ackMutation = useMutation({
    mutationFn: ({
      ...
      customFetch(`/api/notifications/${id}`, {
        method: "PATCH",
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
            ...
            <span className="text-xs uppercase tracking-wide">Activités planifiées</span>
          </div>
          <div className="text-2xl font-bold">{totalPlanned}</div>
          {totalSpecial > 0 && (
            <div className="text-xs text-amber-400 mt-1">{totalSpecial} maintenance/nettoyage</div>
          )}
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
            <Activity className="h-4 w-4" />
            <span className="text-xs uppercase tracking-wide">Saisies du jour</span>
          </div>
          <div className="text-2xl font-bold">{entries?.length ?? 0}</div>
          <div className="text-xs text-muted-foreground mt-1">
            {validatedEntries.length} validée{validatedEntries.length !== 1 ? "s" : ""}
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
        ...
        <div
          className={cn(
            ...
            criticalNotifs.length > 0 ? "bg-red-500/10 border-red-500/30" : "bg-card border-border",
          )}
        >
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
            <Bell className="h-4 w-4" />
            <span className="text-xs uppercase tracking-wide">Alertes ouvertes</span>
          </div>
          <div
            className={cn("text-2xl font-bold", criticalNotifs.length > 0 ? "text-red-400" : "")}
          >
            {openNotifs.length}
          </div>
          {criticalNotifs.length > 0 && (
            <div className="text-xs text-red-400 mt-1">
              {criticalNotifs.length} critique{criticalNotifs.length > 1 ? "s" : ""}
            </div>
          )}
        </div>
      </div>
                      ...
                      <tr key={p.id} className="hover:bg-muted/30">
                        <td className="px-4 py-3">
                          <span className="text-xs font-medium">
                            {p.activityType.replace(
                              "Conditionnement secondaire & tertiaire",
                              "Cond. sec.",
                            )}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right font-mono text-xs">
                          {planned > 0
                            ? planned.toLocaleString("fr-FR") + " " + (p.plannedUnit ?? "")
                            : "—"}
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-xs">
