...
function statusBadge(status: string) {
  ...
  return (
    <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap", s.class)}>
      {s.label}
    </span>
  );
}
...
const CustomTooltip = ({
  ...
  active?: boolean;
  payload?: Array<{ payload: DowntimeParetoItem; value: number; dataKey: string }>;
}) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-3 text-xs shadow-lg max-w-[220px]">
      ...
      <div>
        Durée : <span className="font-medium text-foreground">{d.totalMinutes} min</span>
      </div>
      <div>
        Occurrences : <span className="font-medium text-foreground">{d.occurrences}</span>
      </div>
      <div>
        Part : <span className="font-medium text-foreground">{d.percentage.toFixed(1)}%</span>
      </div>
      <div>
        Cumulé :{" "}
        <span className="font-medium text-foreground">{d.cumulativePercentage.toFixed(1)}%</span>
      </div>
    </div>
  );
};
...
function TrsPeriodView({
  type,
  year,
  equipmentId,
}: {
  type: "hebdomadaire" | "annuel";
  year: number;
  equipmentId?: string;
}) {
  ...
  useEffect(() => {
    ...
    const url =
      type === "annuel"
        ? `/api/dashboard/annual-trs?${params}`
        : `/api/dashboard/weekly-trs?${params}`;
    customFetch<
      Array<{
        monthLabel?: string;
        weekLabel?: string;
    ...
    >(url)
      .then((d) =>
        setData(
          d.map((r) => ({
            label: r.monthLabel ?? r.weekLabel ?? "",
            trs: r.trs,
            TRG: r.TRG ?? null,
            DO: r.DO,
            TP: r.TP,
  ...
  const avg = (k: keyof PeriodPoint) =>
    withData.length > 0
      ? withData.reduce((s, d) => s + ((d[k] as number) ?? 0), 0) / withData.length
      : null;
            ...
            return (
              <div key={label} className="bg-card border border-border rounded-xl p-4">
                ...
                <div
                  ...
                  style={{ color: trsColor(v !== null ? v / 100 : null) }}
                >
                  {v !== null ? `${v.toFixed(1)}%` : "—"}
                </div>
              </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={data} margin={{ top: 8, right: 20, left: 0, bottom: 4 }}>
            ...
            <ReferenceLine
              ...
              stroke="#f59e0b"
              strokeDasharray="4 4"
              label={{ value: "75%", position: "right", fontSize: 9, fill: "#f59e0b" }}
            />
              ...
              <tr key={i} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                ...
                {(["trs", "TRG", "DO", "TP", "TQ"] as const).map((k) => (
                  <td
                    ...
                    style={{ color: trsColor(row[k] !== null ? (row[k] as number) / 100 : null) }}
                  >
                    {row[k] !== null ? (
...
export default function AnalysisPage() {
  ...
  const paretoData = useMemo(() => {
    if (!paretoRaw) return [];
    return (paretoRaw as DowntimeParetoItem[]).slice(
      0,
      groupBy === "type" ? 2 : groupBy === "famille" ? 5 : 12,
    );
  }, [paretoRaw, groupBy]);
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(
      (e: { shift: string }) => shift === "Tous" || e.shift === shift,
    ) as typeof entries;
  }, [entries, shift]);
  ...
  useEffect(() => {
    ...
    customFetch<MonthlyKpisResult>(`/api/dashboard/monthly-kpis?${p}`)
      .then(setMonthlyKpis)
      .catch(() => setMonthlyKpis(null))
  ...
  useEffect(() => {
    if (period !== "mensuel" || !equipmentId) {
      setDailySummary(null);
      return;
    }
    const p = new URLSearchParams({ equipmentId, year: String(year), month: String(month) });
    customFetch<DailySummaryResult>(`/api/daily-entries/monthly-summary?${p}`)
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Exploration détaillée des saisies de production
          </p>
        </div>
        {period === "mensuel" && (
      ...
      <div className="bg-card border border-border rounded-xl p-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={String(year)} onValueChange={(v) => setYear(Number(v))}>
            ...
            <SelectContent>
              {Array.from({ length: now.getFullYear() - 2024 }, (_, i) => 2025 + i).map((y) => (
                <SelectItem key={y} value={String(y)} className="py-3">
                  {y}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Select
            ...
            onValueChange={(v) => setEquipmentId(v !== "all" ? v : undefined)}
          >
            ...
            <SelectContent>
              ...
              {(equipments ?? [])
                ...
                .map((e) => (
                  <SelectItem key={e.id} value={String(e.id)} className="py-3">
                    {e.name}
                  </SelectItem>
                ))}
            </SelectContent>
          ...
          {period === "mensuel" && (
            <>
              ...
              <Select value={status} onValueChange={setStatus}>
                ...
                <SelectContent>
                  <SelectItem value="validated" className="py-3">
                    Validées
                  </SelectItem>
                  <SelectItem value="submitted" className="py-3">
                    Soumises
                  </SelectItem>
          ...
          {monthlyKpis && (
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs text-muted-foreground">
                Objectif :{" "}
                <span className="font-semibold text-foreground">{monthlyKpis.trsObjective}%</span>
                {" · "}tR total :{" "}
              ...
              : monthlyKpis
                ? (
                    [
                      {
                        label: "TRS Consolidé",
                        value: monthlyKpis.trs,
                        sub: `tU / tR · obj ${monthlyKpis.trsObjective}%`,
                      },
                    ...
                    ] as { label: string; value: number | null; sub: string }[]
                  ).map(({ label, value, sub }) => (
                    <div key={label} className="bg-card border border-border rounded-xl p-4">
                      <div className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                        {label}
                      ...
                      <div
                        className="text-2xl font-bold mt-1"
                        style={{ color: trsColor(value !== null ? value / 100 : null) }}
                      >
          ...
          <div className="px-5 py-3 border-b border-border flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm">
                Détail journalier — {MONTHS[month - 1]} {year}
              </span>
              <span className="text-xs text-muted-foreground">
                {dailySummary.daysWithEntries} fiche{dailySummary.daysWithEntries !== 1 ? "s" : ""}{" "}
                / {dailySummary.daysInMonth} jours
              </span>
            </div>
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <span>
                Σ tR :{" "}
                <span className="font-semibold text-foreground">
                  {Math.round(dailySummary.totalTR)} min
                </span>
              </span>
              <span>
                Σ tO :{" "}
                <span className="font-semibold text-foreground">
                  {Math.round(dailySummary.totalTO)} min
                </span>
              </span>
            </div>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {dailySummary.days.map((day) => {
                  ...
                  const sumTU = dayEntries.reduce(
                    (s, e) => s + ((e as { trsMetrics?: { tU?: number } }).trsMetrics?.tU ?? 0),
                    0,
                  );
                  const trsJour = day.tR > 0 ? sumTU / day.tR : null;
                  const dateLabel = new Date(day.entryDate + "T12:00:00").toLocaleDateString(
                    "fr-FR",
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 tabular-nums">
                        {day.tO > 0 ? day.tO : <span className="text-muted-foreground/50">—</span>}
                      </td>
                      <td className="px-3 py-2.5 tabular-nums">
                        {day.tAP > 0 ? (
                          day.tAP
                      ...
                      <td className="px-3 py-2.5 tabular-nums font-semibold">
                        {day.tR > 0 ? day.tR : <span className="text-muted-foreground/50">—</span>}
                      </td>
                      <td className="px-3 py-2.5">
                        {day.status === "validated" ? (
                          <span className="px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 font-medium">
                            Validée
                          </span>
                        ) : (
                      ...
                      <td className="px-3 py-2.5 tabular-nums text-muted-foreground">
                        {dayEntries.length || <span className="text-muted-foreground/40">—</span>}
                      </td>
                      <td className="px-3 py-2.5 tabular-nums">
                        {sumTU > 0 ? (
                          sumTU.toLocaleString("fr-FR")
                        ) : (
                      ...
                      <td
                        ...
                        style={{ color: trsJour !== null ? trsColor(trsJour) : undefined }}
                      >
                        {trsJour !== null ? (
                          `${(trsJour * 100).toFixed(1)}%`
                        ) : (
                          <span className="text-muted-foreground/40">—</span>
                        )}
                      </td>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Select
                value={groupBy}
                onValueChange={(v) => setGroupBy(v as "detail" | "famille" | "type")}
              >
                <SelectTrigger className="h-9 w-52 text-xs">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="detail" className="py-2 text-xs">
                    Détaillé (par catégorie)
                  </SelectItem>
                  <SelectItem value="famille" className="py-2 text-xs">
                    Par famille
                  </SelectItem>
                  <SelectItem value="type" className="py-2 text-xs">
                    Par type (planifié / non planifié)
                  </SelectItem>
                </SelectContent>
              </Select>
              {groupBy !== "type" && (
                <Select
                  value={filterPlanned}
                  onValueChange={(v) => setFilterPlanned(v as "all" | "true" | "false")}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="false" className="py-2 text-xs">
                      Non planifiés seulement
                    </SelectItem>
                    <SelectItem value="true" className="py-2 text-xs">
                      Planifiés seulement
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <>
              <ResponsiveContainer
                ...
                height={
                  groupBy === "type"
                    ...
                    : paretoData.length <= 5
                      ? 200
                      : Math.max(240, paretoData.length * 38)
                }
              >
                <ComposedChart
                  data={paretoData}
              ...
              <div className="mt-2">
                ...
                <ResponsiveContainer width="100%" height={80}>
                  ...
                  >
                    ...
                    />
                    <Tooltip
                      formatter={(v: number) => [`${v.toFixed(1)}%`, "Cumulé"]}
                      contentStyle={{
                        background: "hsl(var(--card))",
                        border: "1px solid hsl(var(--border))",
                        borderRadius: 8,
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-muted-foreground">
                {groupBy === "type" ? (
                  ...
                  Object.entries(FAMILLE_COLORS)
                    .filter(([k]) => !["Planifié", "Non planifié"].includes(k))
                    .map(([label, color]) => (
                  ...
                  <>
                    ...
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-3 rounded-sm bg-amber-500 inline-block" />— Ligne 80%
                      (règle de Pareto)
                    </span>
                  </>
                )}
                  ...
                  <thead>
                    <tr className="border-b border-border bg-muted/40">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left px-3 py-2 font-semibold text-muted-foreground uppercase tracking-wide"
                        >
                          {h}
                        </th>
                      ))}
                        ...
                        <td className="px-3 py-2 font-medium">
                          <span className="inline-flex items-center gap-1.5">
                            <span
                              ...
                              style={{ background: paretoBarColor(row) }}
                            />
                            {row.categoryLabel}
                          </span>
                        </td>
                ...
                {isLoading ? (
                  ...
                  filtered.map((e: ProductionEntryWithDetails) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap max-w-[140px] truncate text-xs">
                        {e.equipmentName}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap max-w-[120px] truncate text-xs">
                        {e.productName}
                      </td>
                      ...
                      <td className="px-3 py-3 text-right whitespace-nowrap tabular-nums">
                        {e.quantityProduced.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right whitespace-nowrap tabular-nums">
                        {e.quantityConforming.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right whitespace-nowrap tabular-nums text-red-500">
                        {e.quantityRejected.toLocaleString()}
                      </td>
                      {["DO", "TP", "TQ", "TRS"].map((k) => (
                        <td
                          key={k}
                          className="px-3 py-3 text-right whitespace-nowrap tabular-nums font-medium"
                          style={{
                            color: trsColor(e.trsMetrics?.[k as keyof typeof e.trsMetrics] ?? null),
                          }}
                        >
                          {e.trsMetrics?.[k as keyof typeof e.trsMetrics] !== undefined
                            ? fmt(e.trsMetrics[k as keyof typeof e.trsMetrics] as number)
                            : "—"}
                        </td>
                      ))}
